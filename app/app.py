# app.py (VERSION AMÉLIORÉE avec Playlists Thématiques + Graphiques Radar)
...
def application_page(analysis, df_results):
    ...
    with col_radar:
        # ⭐ NOUVEAU : Afficher le graphique radar
        st.subheader("📊 Profil Musical")

        # Récupérer les données originales pour avoir les scores non normalisés
...
def playlists_page():
    """⭐ NOUVELLE PAGE : Playlists Thématiques"""
...
def main():
    ...
    page_selection = st.sidebar.radio(
        ...
        (
            ...
            '🎶 Application',
            '🎵 Playlists Thématiques',  # ⭐ NOUVELLE PAGE
            '🌌 Visualisation 3D',
            '📈 Visualisation 2D'
        )
    )
    ...
    st.sidebar.info("• Playlists automatiques\n• Graphiques radar\n• Export CSV")
    ...
    if page_selection == '⚙️ Pipeline de Données':
        ...
        # Pour toutes les autres pages, vérifier que l'analyse est faite
